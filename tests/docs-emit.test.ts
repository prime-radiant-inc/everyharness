...
function tmpFixture(yaml: string): string {
  const dir = mkdtempSync(join(tmpdir(), 'eh-docs-emit-'))
...
describe('gemini installDoc (exact content, kitchen-sink model)', () => {
  it('matches the computed body exactly', () => {
    const files = emitDocs(model, [gemini])
    const file = files.find((f) => f.path === 'docs/install/gemini.md')!
    expect(file.content).toBe(
      [
        '<!-- GENERATED by everyharness — edit everyharness.yaml instead -->',
        '',
        '# Installing kitchen-sink on Gemini CLI',
        '',
        '## What gets emitted',
        '',
        '- `gemini-extension.json`',
        "- `GEMINI.md`, importing the bootstrap skill via Gemini's `@`-import syntax",
        '- a `commands/<name>.toml` slash command for each command (`ks-hello`)',
        '',
        '## Installing',
        '',
        ...
        '',
        '```',
        'gemini extensions install https://github.com/prime-radiant-inc/everyharness',
        '```',
        '',
  ...
  it('matches the computed body exactly', () => {
    ...
    expect(file.content).toBe(
      [
        ...
        '',
        '```json',
        '{ "plugin": ["kitchen-sink@git+https://github.com/prime-radiant-inc/everyharness.git"] }',
        '```',
        '',
...
describe('injectReadme', () => {
  ...
  function tmpReadmeDir(readmeContent?: string): string {
    const dir = mkdtempSync(join(tmpdir(), 'eh-inject-readme-'))
  ...
  it('replaces the region between the markers with a harness/install table, one row per active adapter that implements installDoc', () => {
    const dir = tmpReadmeDir(
      ['# Test Plugin', '', '<!-- everyharness:install:start -->', 'placeholder', '<!-- everyharness:install:end -->', '', 'Footer line.', ''].join(
        '\n',
      ),
    )
  ...
  it('is idempotent: a second call reports {injected: false} and leaves the file byte-identical', () => {
    const dir = tmpReadmeDir(
      ['# Test Plugin', '', '<!-- everyharness:install:start -->', 'placeholder', '<!-- everyharness:install:end -->', ''].join('\n'),
    )
