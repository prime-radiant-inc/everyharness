...
describe('generate', () => {
  it('writes adapter files and a clean manifest', () => {
    ...
    expect(result.adaptersRun).toEqual(['claude-code', 'cursor', 'codex', 'devin', 'kimi', 'gemini', 'opencode', 'pi', 'hermes', 'muse', 'agent-plugins-1.0', 'agents-marketplace'])
  ...
  it('injects the install matrix into README.md between the markers, reports readmeInjected, and leaves validate clean since README.md is not manifest-tracked', () => {
    ...
    expect(readme).toContain('<!-- everyharness:install:start -->')
    expect(readme).toContain('<!-- everyharness:install:end -->')
  ...
  it('respects harnesses.exclude', () => {
    ...
    const result = generate(dir)
    expect(result.adaptersRun).toEqual(['cursor', 'codex', 'devin', 'kimi', 'gemini', 'opencode', 'pi', 'hermes', 'muse', 'agent-plugins-1.0', 'agents-marketplace'])
