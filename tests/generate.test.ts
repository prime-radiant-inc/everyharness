...
describe('generate', () => {
  it('writes adapter files and a clean manifest', () => {
    ...
    expect(result.adaptersRun).toEqual(['claude-code', 'cursor', 'codex', 'devin', 'kimi', 'gemini', 'opencode', 'agent-plugins-1.0', 'agents-marketplace'])
  ...
  it('respects harnesses.exclude', () => {
    ...
    const result = generate(dir)
    expect(result.adaptersRun).toEqual(['cursor', 'codex', 'devin', 'kimi', 'gemini', 'opencode', 'agent-plugins-1.0', 'agents-marketplace'])
